...
function XML:Interpreter()
	...
	local Xpat = "<([%s%S]+)(.-)>(.-)</(%1)>"
	for Type,RawAT,Value,_ in S:gmatch(Xpat) do
		local raw_data = {
			Type = Type, 
			Value = Value,
			Attr = RawAT:split(' ')
		}
		if raw_data.Attr[1] == "" then
	...
	for i = 1, #self.__cache do
		...
		if Valid_Insts[v.Type] then
			local Converted_data = Wrap_data_props(Get_XML_props(v.Attr))
			RBLXCreate(v.Type, Converted_data)
		else
			--Handle specials
		end
	end
